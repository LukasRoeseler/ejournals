...
// daily one, .github/workflows/output-report.yml runs it once a day so the
...
function simplifyWork(work) {
  ...
  // Preprints get their own oa_status category here, the same way the single-user
  // calculator's estimateCost() reports oaStatus: "preprint" -- OpenAlex's own
  // open_access.oa_status would otherwise fold them into "green" or similar.
  ...
  return {
    ...
    // so this reuses OpenAlex's own is_oa flag, except for preprints, which are
    // always free at their host repository without needing that check.
